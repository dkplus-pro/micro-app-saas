...
test('tenant Vite npm scripts are wired for generate-before-dev/build workflow', () => {
  for (const scriptName of ['generate:tenant', 'dev:tenant', 'dev:app1', 'build:vite', 'build:vite:app1']) {
  ...
  assert.match(scripts['build:vite'], /generate|tenant-vite|vite-tenant/i, 'build:vite should route through tenant generation before Vite build');
  assert.match(scripts['build:vite'], /vite/i, 'build:vite should run Vite build');
