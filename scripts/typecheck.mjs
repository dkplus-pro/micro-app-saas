...
const roots = ['scripts'];
const files = roots.flatMap((root) => walk(root)).filter((file) => /\.(ts|mjs)$/.test(file));
let failed = false;
for (const file of files) {
  ...
  const allowedFailure = child.status === 1 && /Missing tenant schema|No tenants selected|Missing release record/.test(`${child.stdout}\n${child.stderr}`);
