...
import type { GeneratedModuleEntry, ModuleKey, NormalizedTenantBuild, PageKey, TenantSchema } from "../../schema/src/index.ts";
...
function normalizeModuleEntries(schema: TenantSchema): GeneratedModuleEntry[] {
  ...
  return orderedKeys.map((key) => ({
    key,
    importPath: MODULE_REGISTRY[key].importPath,
    displayName: MODULE_REGISTRY[key].displayName,
...
export function normalizeTenantBuild(schema: TenantSchema): NormalizedTenantBuild {
  ...
  const pageModules = Object.fromEntries(enabledPages.map(([key, page]) => [key, page.modules ?? []])) as Record<PageKey, TenantSchema["pages"][PageKey]["modules"]>;
  ...
  return {
    ...
    routes,
    pageModules: pageModules as Record<PageKey, TenantSchema["pages"][PageKey]["modules"]>,
    modules,
